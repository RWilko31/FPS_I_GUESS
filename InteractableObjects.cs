...
public class InteractableObjects : MonoBehaviour
{
    ...
    void OnCollisionEnter(Collision collision)
    {
        ...
        string DoorType = collision.gameObject.name;
        if (DoorType.Length > 15)
        {
            DoorType = DoorType.Remove(0, 13);
            int index = DoorType.LastIndexOf("_");
            if (index >= 0) DoorType = DoorType.Substring(0, index);
            if (DoorType == "Door")
            {
