...
public class WeaponManager : MonoBehaviour
{
    ...
    public void createDataArray(int tableSize)
    { WeaponList.WeaponTableRows = new WeaponTableData[tableSize]; }
    public void createNewRow(int row)
    { WeaponList.WeaponTableRows[row] = new WeaponTableData(); }

    /// NOTE: Reading of the weaponTable.CSV has been moved to the csvManager Script
    /// The data itself is still stored in the arrays created in this script
    /// To re-add this functionallity uncomment the function and the variables below
    #region /// uncomment this to re-add add it
    //[SerializeField] public TextAsset weaponTableCSV;
    //[SerializeField] public int No_columns;

    //private void ReadWeaponTableCSV() //Reads the weaponTable.csv file to get weapon info
    //{
    ...
    public List<string[]> saveAmmoData() //puts the curent ammo data in an array to write to the save file
    {
        int current = currentWeaponSlot;
        CycleWeapons(); //Cycle weapons to get most recent ammo data from attack script
        List<string[]> ammoData = new List<string[]>();
        ...
        //set the current weapon ative again
        DeactivateWeapon();
        SetWeaponActive(current);

        return ammoData;
    }
    public List<string[]> saveWeaponData() //puts the current weapon and no_slots in an array to write to the save file
    {
        ...
        headings[0] = "primary"; headings[1] = "no_slots";
        weaponData.Add(headings);

        //add data
        string[] data = new string[2];
        data[0] = currentWeapon; data[1] = NoOfWeaponSlots.ToString();
        weaponData.Add(data);
    ...
    [SerializeField] public int GrappleGunSlot = -1;
    #endregion
    ...
    public void startWeaponManager()
    {
        ...
        //ReadWeaponTableCSV(); //Reads weapon table csv and stores data in array
        csvManager = GDFile.csvManager;
        csvManager.ReadWeaponTableCSV();
    ...
    public string getCurrentWeapon() //returns the name of the weapon currently in use for other scripts
    {
        if (currentWeapon == null) return "none";
        else return currentWeapon;
    }
    public Vector2 checkCurrentWeapons(string weapon) //returns the slot with the chosen weapon in and the total number of used slots
    {
        int No_Slots = 0;
        int weaponSlot = 0;
        int usedSlots = 0;
