...
public class WeaponAndItemScript : MonoBehaviour {
    ...
    private float grappleSpeed = 2.5f;
    public Camera cam;
    ...
    void DefaultInput()
    {
        ...
        Controls.Default.Reload.canceled += ctx => { /* drop objects if held*/ if (!buttonHeld && InteractableObjects.heldObject != null) { InteractableObjects.Interact(); } else if (!buttonHeld && !reloading && allowSwap && ReserveAmmo > 0 && Ammocount != MagSize) { StartCoroutine(Reload()); } buttonHeld = false; };
    ...
    void CheckWeapon()
    {
        ...
        {
            if (fireType == "semi") { Shoot(); ShootRC(); } //semi-auto
            else if (fireType == "auto") fullAutoShoot = true; //Full-auto
            else if (fireType == "burst" && !shootDelay) { StartCoroutine(BurstFire()); } //Burst-fire
            else if (fireType == "spread") { Shoot(); ScatterShootRC(); } //Scattershot
        }
    ...
    void AscendGrappleFunction()
    {
        ...
        if (Mathf.Abs(Vector3.Distance(orientation.position, grapplePoint)) < 1.5f)
        {
            StopGrapple();
            AscendGrapple = false;
        }
    ...
    private void ShootRC()
    {
        ...
        if(Physics.Raycast(ray, rayDistance, EnemyLayer))
        {
            ...
            foreach (RaycastHit obj in hits) { obj.transform.GetComponent<HealthScript>().RaycastDamage(); }
        }
        isShooting = false;
