...
public class CharacterMovementV2 : MonoBehaviour
{
    ...
    public float proneCamPos = -0.5f;
    public float standingCamPos = 0.9f;
    public float crouchCamPos = 0.15f;
    ...
    [SerializeField] public bool slideCoolDownBool = false;
    [SerializeField] private float maxGrapples = 3f;
    [SerializeField] private float no_Grapples;
    [SerializeField] private float grappleHookCoolDown = 3f;
    ...
    [SerializeField] public bool isGroundedJump = false;
    [SerializeField] private bool isGroundedLatch = false;
    private float jumpGroundDistance = 0.52f;
    [SerializeField] public bool inputJump, wallJump, grappleWallRun;
    ...
    [SerializeField] public bool inCrouch = false;
    [SerializeField] public bool inProne = false;
    [SerializeField] public Vector2 crouchLatch = new Vector2(0,0);
    [SerializeField] public bool changePos, canStand;
    public float moveCamPos;
    ...
    [SerializeField] public float slideAngle = 20f;
    ...
    #region Respawn
    [Header("Respawn")]
    [SerializeField] private bool inRespawn = false;
    ...
    {   //Pefroms functions on the first frame
        ...
        PAScript = GetComponent<WeaponAndItemScript>();
        ...
        playerInput = GDFile.gameObject.GetComponent<PlayerInput>();
    ...
    {   //performs updates at a frame dependent rate
        ...
        if (GDFile.isPaused || GDFile.tempPause) return;
        ...
        if (PAScript.AscendGrapple) { useDownForce = false; } //turn off downforce while using grapple
    ...
    {   
        ...
        if (Physics.Raycast(transform.position - new Vector3(0, 0.6f, 0), -orientation.up, 5f, GDFile.deathBarrierLayer)) Respawn();
    ...
    //    Controls.Default.Message.performed += ctx => { if(GDFile.ActiveClient) GDFile.MenuContainer.GetComponent<MainMenuScript>().ShowMesssageGUI(); };

    //    //Menu Controls
    //    Controls.Menu.Resume.performed += ctx => { MMScript.ResumeGame(); };
    //    Controls.Menu.Cancel.canceled += ctx => { MMScript.CancelBackSwitch(); };
    //    Controls.Menu.LeftTab.performed += ctx => { MMScript.LeftTab(); };
    //    Controls.Menu.RightTab.performed += ctx => { MMScript.RightTab(); };
    //    Controls.Menu.Message.performed += ctx => { MMScript.ShowMesssageGUI(); };
    //    Controls.Menu.Submit.performed += ctx => { MMScript.EditText = true; if (MMScript.MessageContainer.activeSelf) { MMScript.EnterMessage(); } };
    //    Controls.Menu.Navigate.performed += ctx => { MMScript.OnInputField(); /*if (MMScript.inDropDown) MMScript.autoScrollDropDown(ctx.ReadValue<Vector2>());*/ }; 
    //}
    #endregion
    ...
    {
        //Debug.Log("respawn");
        inAntigravity = false; AG = 1;
        inSlide = false;
        PAScript.StopGrapple();        
        rb.Sleep();
    ...
    {   //returns the objects the player collides with (used to detect the floor)
        ...
        contact = collision;
        if (collision.gameObject.layer == GDFile.deathBarrierLayer) Respawn();
    }
    ...
    {   //Checks if the player is stood on a slope or not
        ...
        findFloor();
        if (slopeHit.normal != orientation.up) { return true; }
        else { return false; }
    }
    ...
    {   //Finds the surface the player is stood on
        ...
        { Physics.SphereCast(SphereHit.point + (new Vector3(0, 2f, 0) * AG), 0.3f, -orientation.up, out slopeHit, 5f, groundLayer); } //perform a raycast from above the hitpoint so the normal is flat and not angled
    }
    ...
    {   //sets the limits for player abilities e.g max wall runs
        ...
        if (isGroundedRC || PAScript.isGrappling) no_wallJumps = 0f;
        ...
        if (PAScript.isGrappling && !inGrapple) { inGrapple = true; StartGrappleTimer = true; canGrapple = false; } //set variables for grapple and start timer
        if (!PAScript.isGrappling && inGrapple) { inGrapple = false; no_Grapples += 1f; } //reset variables when stop grappling
        ...
        if (isGroundedRC && !PAScript.isGrappling) { no_Grapples = 0f; canGrapple = true; } //reset grapple when grounded and not in use
    ...
    {   //moves the player **may need to add time.deltatime to addforce**
        ...
        else if (isGrounded && onSlope() && CM != 0)
        {
        ...
        else if (xyMove != Vector2.zero) rb.AddForce(slopeMoveDirection.normalized * moveSpeed * moveMultiplier * airSpeed * (CM + 0.0025f), ForceMode.Acceleration); //Air movement
    }
    ...
    {   //changes the speed multiplyer to prevent going too far over maxspeed
        ...
        if (xyMove != Vector2.zero || !isGrounded)
        {
            ...
            else if (xyMove != Vector2.zero) CM = 1 - ((rbSpeed / moveSpeed) / 10f); //slow countermovement when in overspeed
        ...
        #region original cm
        ////If rigidbody speed is larger than movespeed, cancel out the input so you don't go over max speed
        //if ((!inSlide || !slideJump) && xyMove != Vector2.zero || (!isGrounded && !inWallRun))
        //{
        //    if (rbSpeed > moveSpeed + 0.5f && rbSpeed < overSpeed) { CM = 0.8f; inOverSpeed = true; } //Slow down countermovement at high speeds to allow faster movement
        //    else if (rbSpeed > moveSpeed) { CM = 0f; inOverSpeed = false;  } //limit speed if not yet above sprint speed
        //    else { CM = Mathf.MoveTowards(CM, 1f, 0.05f); inOverSpeed = false; }
        //}
        //else { CM = 1; inOverSpeed = false; } //resets countermovement if at a deadstop (No input)
        #endregion
    }
    ...
    {   //deals with wallrun update functions
        if (isGroundedRC) return;

        if (PAScript.isGrappling)
        {
        ...
        if ((wallJump || grappleWallRun) && wallCheck && !PAScript.isGrappling) { wallJump = false; grappleWallRun = false; StartWallRun(); } //allow wall run when jumping to next wall or from grapling
