...
public class CsvManager : MonoBehaviour
{
    ...
    [SerializeField] public int No_columns_it = 13;
    ...
    public class ItemTableData //Stores the table data for each attack
    {
        ...
        public string subType;  //an extra identifier for multiple types of a type e.g different types of armour
    ...
    {   
        ...
        for (int i = 0; i < tableSize; i++)
        {
            ...
            WeaponList.WeaponTableRows[i].weapon = data[No_columns_wt * (i + 1)];
            WeaponList.WeaponTableRows[i].attack = data[No_columns_wt * (i + 1) + 1];
            WeaponList.WeaponTableRows[i].magsize = int.Parse(data[No_columns_wt * (i + 1) + 2]);
            WeaponList.WeaponTableRows[i].reserveAmmo = int.Parse(data[No_columns_wt * (i + 1) + 3]);
            WeaponList.WeaponTableRows[i].fireRate = float.Parse(data[No_columns_wt * (i + 1) + 4]);
            WeaponList.WeaponTableRows[i].reloadTime = float.Parse(data[No_columns_wt * (i + 1) + 5]);
            WeaponList.WeaponTableRows[i].swapTime = float.Parse(data[No_columns_wt * (i + 1) + 6]);
            WeaponList.WeaponTableRows[i].aimPos_x = float.Parse(data[No_columns_wt * (i + 1) + 7]);
            WeaponList.WeaponTableRows[i].aimPos_y = float.Parse(data[No_columns_wt * (i + 1) + 8]);
            WeaponList.WeaponTableRows[i].aimPos_z = float.Parse(data[No_columns_wt * (i + 1) + 9]);
            WeaponList.WeaponTableRows[i].hipPos_x = float.Parse(data[No_columns_wt * (i + 1) + 10]);
            WeaponList.WeaponTableRows[i].hipPos_y = float.Parse(data[No_columns_wt * (i + 1) + 11]);
            WeaponList.WeaponTableRows[i].hipPos_z = float.Parse(data[No_columns_wt * (i + 1) + 12]);
            //Debug.Log(data[No_columns * (i + 1)]); //Reads out each weaponName
        }
    }
    public void SetWeaponData(Weapon weapon) //Give requested weapon data
    {
        //find what row the required data is in
        int row = 0;
        foreach (WeaponTableData WTD in WeaponList.WeaponTableRows)
        {
            if (WTD.weapon == weapon.weapon.ToString()) break;
            else row++;
        }

        //apply data
        float Ax = WeaponList.WeaponTableRows[row].aimPos_x;
        float Ay = WeaponList.WeaponTableRows[row].aimPos_y;
        float Az = WeaponList.WeaponTableRows[row].aimPos_z;
        float Hx = WeaponList.WeaponTableRows[row].hipPos_x;
        float Hy = WeaponList.WeaponTableRows[row].hipPos_y;
        float Hz = WeaponList.WeaponTableRows[row].hipPos_z;
        weapon.aimPos = new Vector3(Ax, Ay, Az);
        weapon.hipPos = new Vector3(Hx, Hy, Hz);

        //check and set if ammo is required
        AttackFunction attackScript = weapon.AttackScript;
        if (attackScript.UsesAmmo())
        {
            attackScript.ammoData = SetAmmoData(weapon.weapon.ToString());
        }
    }
    private AmmoData SetAmmoData(string weaponName) //provides an ammodata class for the weapon given
    {
        AmmoData ad = new AmmoData();
        int row = 0;
        //find what row the required data is in
        foreach (WeaponTableData WTD in WeaponList.WeaponTableRows)
        {
            if (WTD.weapon == weaponName) break;
            else row++;
        }

        //set variables
        int ma = WeaponList.WeaponTableRows[row].magsize;
        ad.MagAmmo = ma; ad.MagMax = ma;
        int ra = WeaponList.WeaponTableRows[row].reserveAmmo;
        ad.ReserveAmmo = ra; ad.ReserveMax = ra;
        float rt = WeaponList.WeaponTableRows[row].reloadTime;
        ad.ReloadTime = rt;
        float fr = WeaponList.WeaponTableRows[row].fireRate;
        ad.RateOfFire = fr;
    ...
    public void ReadItemTableCSV() //get ItemTable data
    {
        ...
        for (int i = 0; i < tableSize; i++)
        {
            ...
            itemList.ItemTableRows[i].itemType = data[No_columns_it * (i + 1) + 8];
            itemList.ItemTableRows[i].subType = data[No_columns_it * (i + 1) + 9];
            itemList.ItemTableRows[i].sellValue = int.Parse(data[No_columns_it * (i + 1) + 10]);
            itemList.ItemTableRows[i].itemDescription = data[No_columns_it * (i + 1) + 12];
            ...
            itemList.ItemTableRows[i].value = SplitString(data[No_columns_it * (i + 1) + 11]);
    ...
    public void getDrops(string entity, HealthScript Hscript)
    {
        ...
        foreach (string item in possibleItemDrops)
        {
            ...
            float rand = Mathf.RoundToInt(UnityEngine.Random.Range(1, dropRate));
            //Debug.Log(rand);
    ...
    public string getItemDescription(string item) //returns the item description for the inventory
    {
        string description = "";
        foreach (ItemTableData itemlist in itemList.ItemTableRows)
    ...
    public List<string> getItemEffect(string item) //returns the values for an accessories effects (def or att etc..)
    {
        List<string> AccFx = new List<string>();
        foreach (ItemTableData itemlist in itemList.ItemTableRows)
        { if (itemlist.itemName == item) { AccFx = itemlist.value; break; } }
        return AccFx;
    }
    public string getItemType(string item) //returns the type for an item e.g item, accessory or armour
    {
        string Type = "";
        foreach (ItemTableData itemlist in itemList.ItemTableRows)
        { if (itemlist.itemName == item) { Type = itemlist.itemType; break; } }
        return Type;
    }
    public string getSubType(string item) //returns the subtype for an item
    {
        string Type = "";
        foreach (ItemTableData itemlist in itemList.ItemTableRows)
        { if (itemlist.itemName == item) { Type = itemlist.subType; break; } }
        return Type;
    }
