...
public class EnemyScript : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            if (Physics.BoxCast(transform.position, new Vector3(4f, 2f, 1f), transform.forward, transform.rotation, targetDistance, playerLayer))
            {
        ...
        else //Start pathfinding when player is not near
        {
                ...
                transform.position.y + nearestCheckPoint.y < 1.5f) //If at checkPoint find next checkpoint and wait for a while
            {
                ...
                if (useCheckpoints)
                {
                    ...
                    {
                        if (currentCheckPoint == 1) nearestCheckPoint = Stop_2.position;
                        if (currentCheckPoint == 2) nearestCheckPoint = Stop_3.position;
                        if (currentCheckPoint == 3) nearestCheckPoint = Stop_4.position;
                        if (currentCheckPoint == 4) nearestCheckPoint = Stop_5.position;
                        if (currentCheckPoint == 5) nearestCheckPoint = Stop_6.position;
                        if (currentCheckPoint == 6) nearestCheckPoint = Stop_1.position;
                        currentCheckPoint++;
                    }
                }
                else { FindRandomPath(); }
    ...
    private void ShootRC()
    {
        ...
        if (Physics.Raycast(this.transform.position, transform.forward + randomisePosition, out hitInfo, bulletDistance, groundLayer)) //Blue Laser
        {
        ...
        hits = Physics.RaycastAll(this.transform.position, transform.forward + randomisePosition, rayDistance, playerLayer); //Yellow Laser
